...
void PintarParticula(const Particula& p){ 
    ...
    switch(p.getTipo()){ 
        ...
        case 3: //Proyectiles
            color = GOLD; 
            DrawRectangle(
                posX - (radio / 2.0),      //Esquina superior izquierda X (para centrarlo y hacerlo más delgado)
                posY - (radio * 1.5),      //Esquina superior izquierda Y (para que parezca que sale desde un punto)
                radio,                      //Ancho (el radio de la partícula)
                radio * 3.0,               //Alto (hacerlo más alto para un efecto de "rayo")
                color
            );
            break;
    } 
...
int main(void) {
    ...
    Minijuego juego(15, 1.0, 1.0); //DEBUG
